...
void bords_carre(float **tab, int n){
  int i, j;
  for (i = 1; i <= n; i++) {
    for (j = 1; j <= n; j++) {
      if (i == j)
...
void mat(float **mat, int n, int type_mat){
  ...
  case 4: hilbert_1(mat, n); break;
  case 5: hilbert_2(mat, n); break;
  case 6: kms(mat, n); break;
  case 7: lehmer(mat, n); break;
  case 8: lotkin(mat, n); break;
  case 9: moler(mat, n); break;
  default: break;
  }
}
